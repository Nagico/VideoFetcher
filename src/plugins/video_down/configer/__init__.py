...
async def got_add_group_id(state: T_State = State(), msg: Message = CommandArg()):
...
async def got_add_confirm(state: T_State = State(), msg: Message = CommandArg()):
...
async def handle_configer_delete(state: T_State = State(), msg: Message = CommandArg()):
...
async def got_delete_group_id(state: T_State = State(), msg: Message = CommandArg()):
    if type(eval(msg.extract_plain_text().strip())) != int:
...
async def got_delete_confirm(state: T_State = State(), msg: Message = CommandArg()):
    state["confirm"] = True if msg.extract_plain_text().strip().lower() == 'true' else False
...
async def handle_configer_update(state: T_State = State(), msg: Message = CommandArg()):
    args = msg.extract_plain_text().strip()  # 首次发送命令时跟随的参数 群号
    if args:
        state["group_id"] = args  # 如果用户发送了参数则直接赋值
...
async def got_update_key(state: T_State = State(), msg: Message = CommandArg()):
...
async def got_update_value(state: T_State = State(), msg: Message = CommandArg()):
...
@configer_get.handle()
async def handle_configer_get(state: T_State = State(), msg: Message = CommandArg()):
    args = msg.extract_plain_text().strip()  # 首次发送命令时跟随的参数 群号
    if args:
        state["group_id"] = args  # 如果用户发送了参数则直接赋值
...
async def got_get_group_id(state: T_State = State(), msg: Message = CommandArg()):
