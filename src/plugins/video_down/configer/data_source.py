...
class Configer:
    ...
    async def read_config_file(self):
        ...
        if not os.path.exists(self.config_path):
            ...
            try:
                if not os.path.exists(os.path.dirname(self.config_path)):  # 文件夹不存在
                    os.makedirs(os.path.dirname(self.config_path))  # 创建文件夹

                async with aiofiles.open(self.config_path, 'w', encoding='utf-8') as f:
                    await f.write(json.dumps({}, ensure_ascii=False, indent=4))  # 创建空配置文件
        ...
        try:
            async with aiofiles.open(self.config_path, 'r', encoding='utf-8') as f:
                self.config_data = json.loads(await f.read())  # 读取配置文件
    ...
    def get_config_data(self, group_id: int = None) -> dict:
        """
        获取配置文件内容
    ...
    def set_config_data(self, group_id: int, video_upload_path: str = None, cover_upload_path: str = None,
                        enable_subscribe: bool = None) -> None:
        """
        设置配置信息
    ...
    def get_subscribe_channel(self, group_id: int):
        """
        获取订阅列表
